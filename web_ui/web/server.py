...
def register_cp():
    """Register a CP in Registry (Web UI → Registry)."""
    data = request.json or {}
    cp_id = (data.get("cp_id") or "").strip()
    city = (data.get("city") or "").strip() or "Madrid"
    price = data.get("price_per_kwh")

    if not cp_id or price is None:
        return jsonify({"success": False, "error": "cp_id and price_per_kwh required"}), 400

    # Normalize CP id to uppercase like CP-001
    cp_id = cp_id.upper()

    try:
        price = float(price)
    except Exception:
        return jsonify({"success": False, "error": "price_per_kwh must be a number"}), 400
    ...
    try:
        ...
        # Registry already returns JSON
        return jsonify(r.json()), r.status_code
    except requests.exceptions.Timeout:
        return jsonify({"success": False, "error": "Registry timeout"}), 504
    except Exception as e:
        return jsonify({"success": False, "error": str(e)}), 500
...
def proxy_weather():
    """Simulate weather alert from Web UI by sending TCP protocol to Central."""
    global ui_client
    data = request.json or {}
    cp_id = (data.get("cp_id") or "").strip().upper()
    alert = (data.get("alert") or "").strip()

    if not cp_id or not alert:
        return jsonify({"success": False, "error": "cp_id and alert required"}), 400

    if not ui_client or not ui_client.is_connected():
        return jsonify({"success": False, "error": "UI monitor not connected to Central"}), 503

    ok = ui_client.send_weather_alert(cp_id, alert)
    if ok:
        return jsonify({"success": True, "cp_id": cp_id, "alert": alert}), 200
    return jsonify({"success": False, "error": "Failed to send WEATHER_ALERT"}), 500
