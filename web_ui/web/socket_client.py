...
class CentralUIClient:
    ...
    def _handle_message(self, fields):
        ...
        if t == MessageTypes.SUPPLY_UPDATE and len(fields) >= 4:
            cp_id = fields[1]
            # Central broadcasts TOTAL values to UI: (kwh_total, amount_total)
            # Keep direct assignment so progress grows correctly each second.
            kwh_total = float(fields[2])
            amount_total = float(fields[3])
            self.state.update_cp(cp_id, {"kwh_delivered": kwh_total, "amount_euro": amount_total})
            return
