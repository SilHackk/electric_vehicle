...
# FIXES (REAL):
# - UI receives cumulative kWh and € (NOT increment-only JSON)
...
# - REST API: /api/register_cp on port 5003
# ============================================================================
...
class EVCentral:
            ...
            with self.lock:
                if cp_id not in self.charging_points:
                    self.charging_points[cp_id] = {
                        ...
                        "location": (lat, lon),
                        "location_name": self._format_location_name(city=city, lat=lat, lon=lon),
                        "price_per_kwh": price,
                        "current_driver": None,
                        "kwh_delivered": 0.0,
                        "amount_euro": 0.0,
    ...
    def _format_location_name(self, city=None, lat=None, lon=None):
        if city and str(city).strip():
            return f"{str(city).strip()}, Lithuania"
        if lat is None or lon is None:
            return "Unknown"
    ...
    def _load_stored_cps(self):
        ...
        for cp in stored:
            ...
            self.charging_points[cp_id] = {
                ...
                "location_name": self._format_location_name(lat=cp["latitude"], lon=cp["longitude"]),
        ...
        with self.lock:
            ...
            for cp_id, cp in self.charging_points.items():
                cps.append([
                    ...
                    cp.get("location_name", self._format_location_name(lat=cp["location"][0], lon=cp["location"][1])),
                ])
        ...
        try:
            while self.running:
                ...
                while buffer:
                    ...
                    socket_type = self.socket_to_type.get(client_socket)

                    # CP runtime traffic is encrypted after registration.
                    # Decrypt first to avoid false-positive plaintext parses.
                    if cp_id and socket_type == "CP":
                    ...
                    # Fallback: plaintext parse (REGISTER, MONITOR, DRIVER, etc.)
        ...
        if entity_type == "CP":
            ...
            with self.lock:
                ...
                cp.update({
                    ...
                    "location": (lat, lon),
                    "location_name": cp.get("location_name", self._format_location_name(lat=lat, lon=lon)),
                    "price_per_kwh": price,
    ...
    def _verify_cp_credentials(self, cp_id, username, password):
        # Registry-free mode: require non-empty credentials and cache locally.
        if not username or not password:
            print(f"[EV_Central] Missing credentials for {cp_id}")
            return False
        with self.lock:
    ...
    def _handle_supply_update(self, fields):
        ...
        with self.lock:
            ...
            if kwh_needed > 0 and cp["kwh_delivered"] >= kwh_needed:
                if not cp.get("charging_complete", False):
                    ...
                    # 🔴 CENTRAL decides to stop charging
                    self._send_to_cp(
                        cp_id,
        ...
        # ✅ CRITICAL: send TOTALS to WEB_UI using protocol message (NOT JSON payload)
        self._broadcast_to_ui_protocol(
